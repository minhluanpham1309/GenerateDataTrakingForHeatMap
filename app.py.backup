...
def generate_record(record_id, url_id, long_data=False):
    ...
    if utm_data['source'] == '-':
        ...
        if long_data:
            # Bắt đầu với UTM parameters cơ bản (QUAN TRỌNG - GIỮ NGUYÊN)
            params = [
            ...
            # Thêm UTM parameters mở rộng
            params.extend([
                f"utm_term={secrets.token_hex(12)}",  # 24 chars
                f"utm_content={secrets.token_hex(15)}",  # 30 chars
                f"utm_id={secrets.token_hex(8)}",  # 16 chars
                f"utm_source_platform={utm_data['source']}_platform",
                f"utm_campaign_id={random.randint(100000, 999999)}",
                f"utm_adgroup={secrets.token_hex(10)}",  # 20 chars
                f"utm_keyword={secrets.token_hex(8)}",  # 16 chars
            ])
            
            # Thêm tracking parameters
            params.extend([
                f"gclid={secrets.token_hex(25)}",  # 50 chars Google Click ID
                f"fbclid={secrets.token_hex(20)}",  # 40 chars Facebook Click ID  
                f"msclkid={secrets.token_hex(18)}",  # 36 chars Microsoft Click ID
                f"ttclid={secrets.token_hex(15)}",  # 30 chars TikTok Click ID
                f"li_fat_id={secrets.token_hex(12)}",  # 24 chars LinkedIn
                f"twclid={secrets.token_hex(14)}",  # 28 chars Twitter Click ID
            ])
            ...
            params.extend([
                ...
                f"session_id={secrets.token_hex(16)}",  # 32 chars
            ])
            
            # Kết hợp thành query string
            query_string = '&'.join(params)
            url = base_url + '?' + query_string
        else:
            # URL ngắn với UTM cơ bản
...
@app.route('/')
def home():
    return '<h1>UTM Generator</h1><a href="/referrer_page">Tạo UTM Data</a>'

@app.route('/referrer_page')
def referrer_page():
    return '''
    <!DOCTYPE html>
    <html>
...
if __name__ == '__main__':
    print("🚀 Server starting at http://localhost:5000/referrer_page")
